...
import { getSpellSlots, isSpellcaster, CLASS_DATA } from '../../data/class-starting-data.js';
...
async function handleUpdate(args: z.infer<typeof UpdateSchema>): Promise<object> {
    ...
    {
        const resolved = characterRepo.findById(args.characterId);
        if (!resolved) throw new Error(`Character ${args.characterId} not found`);
        // FINDINGS #88: guard before ANY write path (repo or raw SQL).
        if (args.expectName !== undefined && resolved.name.toLowerCase() !== args.expectName.toLowerCase()) {
            throw new Error(`GUARD REFUSAL: character ${args.characterId} is "${resolved.name}", not "${args.expectName}" — NOTHING was written.`);
        }
        args = { ...args, characterId: resolved.id };
    }
...
const definitions: Record<CharacterAction, ActionDefinition> = {
    ...
    options: {
        schema: z.object({ action: z.literal('options') }),
        handler: async () => {
            // #67-D: creation options read from CLASS_DATA — single source, never
            // a hardcoded list. PC-gate rule stated where the caller will see it.
            const classes = Object.entries(CLASS_DATA).map(([key, d]) => ({
                class: key,
            ...
            return {
                ...
                message: `${classes.length} provisioning classes available: ${classes.map(c => c.class).join(', ')}`
            };
        },
        aliases: ['creation_options', 'classes'],
        description: '#67-D: List creation options — every provisioning class from CLASS_DATA with hit die and caster flag, plus the PC-gate rule'
    },
    adjust_pool: {
...
Aliases: new/add/spawn->create, fetch/find->get, modify/edit->update`,
    ...
    inputSchema: z.object({
        action: z.string().describe('Action: create, get, update, list, delete, kill, add_xp, get_progression, level_up'),
