...
const ACTIONS = ['give', 'remove', 'transfer', 'use', 'equip', 'unequip', 'get', 'get_detailed', 'add_currency', 'attach', 'detach', 'adjust_condition', 'adjust_charges', 'coat', 'uncoat'] as const;
...
function deriveArmorClass(
    ...
    for (const row of rows) {
        ...
        const p = (t?.properties ?? {}) as { baseAC?: number; maxDexBonus?: number; acBonus?: number };
        if (row.slot === 'armor' && typeof p.baseAC === 'number') {
            const maxDex = p.maxDexBonus !== undefined ? Number(p.maxDexBonus) : 99;
            base = p.baseAC + (maxDex > 0 ? Math.min(dexMod, maxDex) : 0);
            baseDesc = `${t?.name ?? 'armor'} base ${p.baseAC}${maxDex < 99 ? ` + DEX(max ${maxDex})` : ' + DEX'}`;
        } else if (typeof p.acBonus === 'number' && p.acBonus !== 0) {
            bonus += p.acBonus;
        ...
        handler: async (params: z.infer<typeof EquipSchema>) => {
            ...
            if (character) {
                ...
                if (derived.ac !== character.ac) {
                    ...
                    acChange = `AC ${character.ac} → ${derived.ac} (${derived.parts.join(' + ')})`;
                } else {
                    acChange = `AC unchanged (${derived.ac}: ${derived.parts.join(' + ')})`;
                }
            }
